...
# 단일 파일: Streamlit 데이터 매니저 (UX 리뉴얼, 모바일 친화) + 샘플 데이터(내장) + APScheduler Fallback
# - API v1(GET-only) / 샘플 모드 지원
# - 자동 동기화: APScheduler 있으면 06:00 KST cron, 없으면 "앱 열릴 때 하루 1회" 대체 수행
# - 탭 UI(오버뷰/탐색/변경), 큰 버튼/간결 필터, 이모지 액션 칩, CSV 다운로드
#
# 실행 예시
#   pip install streamlit pandas requests pytz python-dotenv   # (apscheduler 선택)
#   USE_SAMPLE=1 streamlit run fmw_dm_single_ux2.py
#   USE_SAMPLE=0 API_BASE="http://backend.example.com/v1" API_KEY="..." streamlit run fmw_dm_single_ux2.py
...
try:
    ...
    BackgroundScheduler = None  # 패키지 없으면 Fallback 로직 사용
...
# ===================== UI (모바일 최적화) =====================
st.set_page_config(page_title="FMW 데이터 매니저 (리뉴얼)", layout="wide", page_icon="🗂️")
st.markdown("""
    <style>
...
st.title("FMW 데이터 매니저 · UX 리뉴얼")
st.caption("v1 GET-only · 자동 동기화 · 모바일 친화")
...
with tab_ov:
    ...
    col1.metric("마지막 동기화(KST)", idx_last)
    col2.metric("추가(ADD)", added)
    col3.metric("수정(UPD)", updated)
    col4.metric("삭제(REM)", removed)

    # 선택된 세트의 snapshot 통계
    if sel_group and sel_feature:
        df = pd.DataFrame(_load_df(f"records__{sel_group}__{sel_feature}"))
        if not df.empty:
            ...
            st.info("선택한 그룹/피처의 캐시가 아직 없습니다. 자동 동기화 이후 확인하세요.")
    else:
...
with tab_changes:
    ...
    rc = load_recent_changes(limit=300)
