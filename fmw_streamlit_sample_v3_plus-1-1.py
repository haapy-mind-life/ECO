...
Run:
    streamlit run fmw_streamlit_sample_v3_plus-1.py
"""
...
def aggregate_summary(audit: pd.DataFrame, days: int, yesterday: bool=False) -> Dict:
    ...
    if df.empty:
        ...
        # Ensure required columns exist even if absent in this window
        for col in ("created","updated","deleted"):
            if col not in merged.columns:
                merged[col] = 0

    merged["errors"] = [RND.randint(0, 2) for _ in range(len(merged))]
    # enforce integer dtype for counts
    for col in ("created","updated","deleted","errors"):
